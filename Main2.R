...
english <- as.vector(english[!english==""])

set.seed(90210)
corpus <- c(blog,twitter,news)
train <- sample(1:length(corpus), size = round(length(corpus)*0.2))
corpus <- tibble(text = corpus[train]) %>%
        mutate(line = row_number(), .before = "text")

#clean up ram
rm(blog,news,twitter)
voc <- english[!english%in%prof]
unigram <- corpus %>% unnest_tokens(ngram, text, token = "ngrams", n = 1) %>%
        filter(ngram %in% voc) %>% count(ngram)

#decreases the voc size
voc <- unique(unigram$ngram)
